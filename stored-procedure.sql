...
sp: BEGIN
    ...
    ELSE
        -- SELECT 'MOVIE DOES NOT EXIST';
        SELECT COUNT(*) INTO @currmaxmovieid FROM movies;
	 SELECT @integermovieid := CAST(@currmaxmovieid AS SIGNED) + 1;
        SET movieid = CONCAT('ZT', @integermovieid); -- the id of the newly created movie
        INSERT INTO movies (id, title, year, director) VALUES(movieid, newTitle, newYear, newDirector);
    ...
    ELSE
        -- SELECT 'STAR DOES NOT EXIST';
        SELECT COUNT(*) INTO @currmaxstarid FROM stars; 
        SET @integerstarid := CAST(@currmaxstarid AS SIGNED) + 1;
        SET starid = CONCAT('ABC', @integerstarid); -- the id of the newly created star
